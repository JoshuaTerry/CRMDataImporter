...
{
    public class FaxMapper : MapperBase<Fax>
    {
        public FaxMapper(bool update)
            : base(SourceDatabaseEnum.CRM3, update) 
        {
            this.Query = @" select 
                                ...
                                (select  
	                                ...
	                                where Party.ActivityId = Fax.ActivityId 
	                                FOR XML Raw('Party'), TYPE, Root('Parties')) as 'Parties' ,	
	                                cast(Fax.DirectionCode as bit) as 'DirectionCode', 
                                Fax.[Subject],  
                                Fax.[Description],
                                Fax.RegardingObjectId,
                                Fax.RegardingObjectTypeCode as 'RegardingObjectIdMapperTypeCode',  
                                su.DomainName as 'OwnerId',
                                Fax.ActualDurationMinutes, 
                                Fax.PriorityCode,
                                Fax.ScheduledEnd,
                                Fax.CoverPageName,  
                                Fax.CreatedOn as 'OverriddenCreatedOn' 
                                from Fax 
                                inner join SystemUser su
	                                on su.SystemUserId = Fax.OwnerId 
                            ";
        }
        ...
        public override bool IsImportable(Fax entity)
        {
            return !DestinationKeyExists(entity.ActivityId.Value,"Fax") && (entity.RegardingObjectId == null ||
